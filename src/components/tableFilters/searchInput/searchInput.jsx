...
const TableSearch = ({
  ...
  const getSuggestions = (value) => {
    ...
    const names = [...data.map((item) => item?.name || item?.fileName)];
    const descriptions = [
      ...data.map((item) => item?.description || item?.title),
    ];
    const combinedItems = names.concat(descriptions);
    return inputLength === 0
      ? []
      : combinedItems.filter((value) => regex.test(getSuggestionValue(value)));
  };
